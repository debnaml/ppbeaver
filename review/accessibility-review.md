...
| Priority | Action                                                                            |
| -------- | --------------------------------------------------------------------------------- |
| 🟠 1     | Audit and fix colour contrast ratios across all text/background pairs             |
| 🟠 2     | Add visible focus indicators to all custom interactive elements                   |
| 🟠 3     | Add `aria-describedby`, `aria-invalid`, and `role="alert"` to form errors         |
| 🟡 4     | Add hidden navigation and `<header>` landmark to legal/404 pages                  |
| 🟡 5     | Fix heading hierarchy (add `<h1>` to legal pages, flatten OrbitShowcase headings) |
| 🟡 6     | Add ARIA tabs pattern or improved keyboard nav to OrbitShowcase                   |
| 🟡 7     | Update modal title / add aria-live on step change                                 |
| 🟡 8     | Fix duplicate/mismatched image alt attributes                                     |
| 🟡 9     | Ensure footer nav links meet 24×24 CSS px minimum touch target                    |
| 🟢 10    | Use explicit `aria-hidden="true"` throughout                                      |
| 🟢 11    | Add `aria-busy` to hero section during load; review scroll lock timeout           |
